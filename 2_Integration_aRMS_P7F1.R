...
## Subset subtypes
P7F1.combined <- subset(PDX.combined, subset = fusion == 'PAX7::FOXO1')
P3F1.combined <- subset(PDX.combined, subset = fusion == 'PAX3::FOXO1')


##################################################################
#  (1) Integration with RPCA correction P7F1:  #
##################################################################

## Split object by name (= sample)
PDX.list <- SplitObject(P7F1.combined , split.by = "name")
PDX.list
...
saveRDS(PDX.integrated, file = "/mnt/Sara/write/Danielli_Patel_Langenau_RPCA_ARMS_P7F1.rds")



##################################################################
#  (2) Scoring P7F1 cells for gene sets:  #
##################################################################
...
PDX.integrated <- readRDS("/mnt/Sara/write/Danielli_Patel_Langenau_RPCA_ARMS_P7F1.rds")
PDX.integrated 
DefaultAssay(PDX.integrated) <- "integrated"
...
PDX.integrated <- AddModuleScore(object = PDX.integrated, assay = 'RNA', features = Wei_FN_genes, ctrl = 100, name = "Wei_FN_genes")
PDX.integrated <- AddModuleScore(object = PDX.integrated, assay = 'RNA', features = Wei_FP_genes, ctrl = 100, name = "Wei_FP_genes")
...
###### Save integrated dataset ###########
saveRDS(PDX.integrated, file.path(base_dir,"write/Danielli_Patel_Langenau_RPCA_ARMS_P7F1_scores.rds"))
PDX.integrated <- readRDS(file.path(base_dir, "write/Danielli_Patel_Langenau_RPCA_ARMS_P7F1_scores.rds"))
