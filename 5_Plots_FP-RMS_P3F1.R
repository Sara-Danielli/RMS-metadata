...
names(col_cluster_names_aggregate) <- c('Progenitor', 'Proliferative', 'Ground', 'Differentiated', 'Neuronal', 'Apoptosis')
...
# combine lists
FP_genes <- c(FP_target_signature)
...
VlnPlot(P3F1, 
  ...
  theme_vln + labs(y = 'Module score, AU')
ggsave(file.path(analysis_dir, paste0("24_VlnPlot_FPsignature.pdf")), width=3.5, height=4.5)
...
# Score tumor programs 
  P3F1 <- AddModuleScore(object = P3F1, assay = 'RNA', features = marker_list, name = names(marker_list))
  P3F1 <- ScaleData(P3F1)
  
  # rename metadata names of scores
  col_start <- length(colnames(P3F1@meta.data)) - length(names(marker_list)) + 1
  # identify number of last column with metadata scores
  col_end <- length(colnames(P3F1@meta.data))
  # rename columns with score name
  colnames(P3F1@meta.data)[col_start:col_end] <- names(marker_list)
  
# Dotplot
  DotPlot(P3F1, 
          features = c('Progenitor', 'Proliferative', 'Ground', 'Differentiated', 'Neuronal', 'Apoptosis'), 
          group.by = 'Cluster assignment',
          assay = 'RNA', 
          #cols = c("white", "red3"),
          scale = T,
