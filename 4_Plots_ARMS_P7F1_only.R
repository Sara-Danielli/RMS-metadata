...
## read file
P7F1.integrated <- readRDS(file.path(base_dir, "write/Danielli_Patel_Langenau_RPCA_ARMS_P7F1_20230713.rds"))
DefaultAssay(P7F1.integrated) <- "integrated"
...
col_cluster_names_aggregate <- c("#7FBC41FF", '#FFAD72FF', '#FFE5CCFF', "#8E0152FF",'#66C5CCFF')

## Order factors
P7F1.integrated$origin <- factor(x = P7F1.integrated$origin, levels = c("Wei et al.", "Patel et al.", "Danielli et al.", "Weng et al."))
P7F1.integrated$model <- factor(x = P7F1.integrated$model, levels = c("Patient", "O-PDX", "Primary culture", "Cell line"))
P7F1.integrated$subtype <- factor(x = P7F1.integrated$subtype, levels = c("eRMS", "aRMS"))
P7F1.integrated$name <- factor(x = P7F1.integrated$name, levels = c('20082',  'aRMS-1',  'aRMS-2', 'aRMS-3', 
...
P7F1.integrated$cluster_names <- factor(x = P7F1.integrated$cluster_names, 
                                       levels = c('Progenitor', 'Proliferative','DNA replication', 'Ground', 'Differentiated', 'Neuronal'))

P7F1.integrated$cluster_names_aggregate <- factor(x = P7F1.integrated$cluster_names_aggregate, 
                                                 levels = c('Progenitor', 'Proliferative', 'Ground', 'Differentiated', 'Neuronal'))
...
for (a in 1:length(variables2)) {
  DimPlot(P7F1.integrated, reduction = "pca_rpca", group.by = variables2[[a]], cols = colors_variables2[[a]], pt.size = 4, 
...
p <- FeaturePlot(P7F1.integrated, features = scores, reduction = "pca_rpca", combine = FALSE, sort.cell = TRUE, pt.size = 4, raster=TRUE)
for(i in 1:length(p)) {
...
p <- FeaturePlot(P7F1.integrated, features = myogenesis_markers, reduction = "pca_rpca", combine = FALSE, sort.cell = TRUE, pt.size = 4, raster=TRUE)
for(i in 1:length(p)) {
  p[[i]] <- p[[i]] + NoAxes() + scale_colour_distiller(palette="RdBu")
}
...
for (a in 1:length(variables2)) {
  DimPlot(P7F1.integrated, reduction = "umap_rpca", group.by = variables2[[a]], cols = colors_variables2[[a]], pt.size = 4, raster=TRUE, shuffle=TRUE, raster.dpi = c(1012, 1012)) + NoLegend()
  ggsave(file.path(analysis_dir, paste0("5_UMAP_",names(variables2)[a],"_no_legend.pdf")), width=4, height=4, dpi=300)
}

for (a in 1:length(variables2)) {
  DimPlot(P7F1.integrated, reduction = "umap_rpca", group.by = variables2[[a]], cols = colors_variables2[[a]], pt.size = 4, raster=TRUE, shuffle=TRUE, raster.dpi = c(1012, 1012)) 
  ggsave(file.path(analysis_dir, paste0("5_UMAP_",names(variables2)[a],".pdf")), width=12, height=5, dpi=300)
}


# UMAP plot Louvain clusters 
Idents(P7F1.integrated) = P7F1.integrated$cluster_names
DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = FALSE, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012)) + NoLegend() + NoAxes()
ggsave(file.path(analysis_dir, paste0("7_UMAP_clusters.pdf")), width=5, height=5, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = 1, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012)) + NoLegend() + NoAxes()
ggsave(file.path(analysis_dir, paste0("7_UMAP_clusters_labels.pdf")), width=, height=, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = 1, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012)) + NoAxes()
ggsave(file.path(analysis_dir, paste0("7_UMAP_clusters_legend.pdf")), width=5, height=4, dpi=300)


# UMAP plots of scores
p <- FeaturePlot(P7F1.integrated, features = scores, reduction = "umap_rpca", combine = FALSE, sort.cell = TRUE, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012))
for(i in 1:length(p)) {
...
DimPlot(P7F1.integrated, reduction = "umap_rpca", pt.size = 4, raster=TRUE, group.by = 'Phase', cols = c('black', 'gray76', 'grey41'), raster.dpi = c(1012, 1012), shuffle=TRUE)  + NoAxes()
ggsave(file.path(analysis_dir, "9_UMAP_cellcycle.pdf"), width=6, height=5, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca", pt.size = 4, raster=TRUE, group.by = 'Phase', cols = c('black', 'gray76', 'grey41'), raster.dpi = c(1012, 1012), shuffle=TRUE) + NoLegend()  + NoAxes()
...
p <- FeaturePlot(P7F1.integrated, features = myogenesis, reduction = "umap_rpca", combine = FALSE, sort.cell = TRUE, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012))
for(i in 1:length(p)) {
  p[[i]] <- p[[i]] + NoAxes() + scale_colour_distiller(palette="RdBu")
}
cowplot::plot_grid(plotlist = p, ncol=3)
...
p <- FeaturePlot(P7F1.integrated, features = myogenesis_short, reduction = "umap_rpca", combine = FALSE, sort.cell = TRUE, pt.size = 1, raster=TRUE)
for(i in 1:length(p)) {
  p[[i]] <- p[[i]] + NoAxes() + scale_colour_distiller(palette="RdBu")
}
cowplot::plot_grid(plotlist = p, ncol=4)
...
Idents(P7F1.integrated) = 'cluster_names'
DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = FALSE, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012), shuffle=TRUE) + NoLegend() + NoAxes()
ggsave(file.path(analysis_dir, "7_UMAP_clusters_name.pdf"), width=4, height=4, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = 1, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012), shuffle=TRUE) + NoLegend() + NoAxes()
ggsave(file.path(analysis_dir, "7_UMAP_clusters_labels_name.pdf"), width=4, height=4, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = 1, cols = col_cluster_names, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012), shuffle=TRUE) + NoAxes()
...
Idents(P7F1.integrated) = 'cluster_names_aggregate'
DimPlot(P7F1.integrated, reduction = "umap_rpca",  label = 0, cols = col_cluster_names_aggregate, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012), shuffle=TRUE)  + NoAxes()
ggsave(file.path(analysis_dir, "7b_UMAP_clusters_aggregates.pdf"), width=5.5, height=4, dpi=300)

DimPlot(P7F1.integrated, reduction = "umap_rpca",label = 0, cols = col_cluster_names_aggregate, pt.size = 4, raster=TRUE, raster.dpi = c(1012, 1012), shuffle=TRUE) + NoLegend() + NoAxes()
...
# Read markers
P7F1.integrated.markers <- read.csv(file.path(analysis_dir, "12_markers_SCT.csv"))

order_cluster <- c('Progenitor', 'Proliferative','DNA replication', 'Ground', 'Differentiated', 'Neuronal')

P7F1.integrated.markers2 <- P7F1.integrated.markers %>%
  mutate(cluster =  factor(cluster, levels = order_cluster)) %>%
  arrange(cluster) 

P7F1.integrated.markers2 <- P7F1.integrated.markers %>% group_by(cluster)

number_cells <- c(100, 200, 400)
names(number_cells) <- c('100', '200', '400')

## Heatmap plot of all markers subsetting to 100/200/400 cells
Idents(P7F1.integrated) = 'seurat_clusters'
for (a in 1:length(number_cells)) {
  DoHeatmap(subset(P7F1.integrated, downsample = number_cells[[a]]), features = P7F1.integrated.markers2$gene, draw.lines = TRUE, angle=30, size=4, group.colors = col_cluster_names, raster=TRUE)  +
...
top10 <- P7F1.integrated.markers2 %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
for (a in 1:length(number_cells)) {
  DoHeatmap(subset(P7F1.integrated, downsample = number_cells[[a]]), features = top10$gene, draw.lines = TRUE, angle=30, size=4, group.colors = col_cluster_names, raster=TRUE)  +
    scale_fill_gradient2(low = rev(c('#d1e5f0','#67a9cf','#2166ac')), mid = "white", high = rev(c('#b2182b','#ef8a62','#fddbc7')))
...
top50 <- P7F1.integrated.markers2 %>% group_by(cluster) %>% top_n(n = 50, wt = avg_log2FC)
DoHeatmap(P7F1.integrated, features = top50$gene, draw.lines = TRUE, angle=30, size=4, group.colors = col_cluster_names, raster=TRUE)  +
  scale_fill_gradient2(low = rev(c('#d1e5f0','#67a9cf','#2166ac')), mid = "white", high = rev(c('#b2182b','#ef8a62','#fddbc7')))
ggsave(file.path(analysis_dir, "8_Heatmap_down_top50.png"), width=8, height=10, dpi=300)


## Correlation matrix clusters
SCpubr::do_CorrelationPlot(sample = P7F1.integrated, group.by = 'cluster_names', cell_size = 10)
p

## Bar plot clusters distribution
variables <- list(P7F1.integrated$name, P7F1.integrated$subtype, P7F1.integrated$model, P7F1.integrated$origin, P7F1.integrated$fusion)
names(variables) <- c('name', 'model', 'origin', 'fusion')

# Bar plot clusters
for (a in 1:length(variables)) {
  plot_bar(P7F1.integrated, variables[[a]], P7F1.integrated$cluster_names, col_cluster_names) 
...
for (a in 1:length(variables)) {
  plot_bar(P7F1.integrated, variables[[a]], P7F1.integrated$cluster_names_aggregate, col_cluster_names_aggregate) 
...
plot_bar(P7F1.integrated, P7F1.integrated$name, P7F1.integrated$cluster_names_aggregate, col_cluster_names_aggregate) 
...
plot_bar(P7F1.integrated, P7F1.integrated$cluster_names, P7F1.integrated$Cycling_prop, c('black', 'grey')) 
...
for (a in 1:length(Vln_scores)) {
  VlnPlot(P7F1.integrated,features = Vln_scores[[a]], group.by = 'name',  split.by = 'subtype',  pt.size=0,  cols = col_subtype) 
  ggsave(file.path(analysis_dir, paste0("15_Vln_plot_scores_",names(Vln_scores)[a],".pdf")), width=15, height=5, dpi=300)
}


## FeaturePlot
FeatureScatter(P7F1.integrated,
...
Idents(P7F1.integrated) = 'cluster_names'
P7F1.integrated_small <- subset(P7F1.integrated, downsample = 500)

SCpubr::do_CorrelationPlot(sample = P7F1.integrated_small, cell_size = 10)



## Violin plot of muscle lineage and proliferation scores across different models

P7F1.integrated_number_cells <- P7F1.integrated@meta.data %>% 
  group_by(P7F1.integrated@meta.data$model, P7F1.integrated@meta.data$name) %>% 
  summarise(n = n()) 
P7F1.integrated_number_cells

compare_means(DS.Difference_common ~ model, data = P7F1.integrated[[]])
p1 <- VlnPlotScoresModel(P7F1.integrated, features = 'DS.Difference_common', y = 1.4) +
  stat_compare_means(method = 't.test', size = 3)
p2 <- VlnPlotScoresModel(P7F1.integrated, features = 'Common_proliferative1', y = 1.8) +
  stat_compare_means(method = 't.test', size = 3)

plot_grid(p2, p1, ncol = 2, align = "h", axis = "tb")
ggsave(file.path(analysis_dir,"19_VlnPlot_statistics.pdf"), width=3.5, height=3.5, dpi=300)
...
ggsave(file.path(analysis_dir, paste0("23_DotPlot_FPsignature.pdf")), width=4.5, height=6, dpi=300)
